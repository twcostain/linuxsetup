...
if __name__ == '__main__':
    ...
    parser.add_argument('--theme', required=True, action='store', choices=['True', 'False'])
    ...
    if ARGS.theme:
        print("Installing Arc Theme and Icons")
        try:
            subprocess.run(['./setuptheme.sh'], check=True)
        except subprocess.CalledProcessError as err:
            print("Installing arc theme failed with error code: {}".format(err.returncode))
    if ARGS.latex:
